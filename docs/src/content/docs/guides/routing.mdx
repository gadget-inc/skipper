...
The `metadata` field is an opaque string passed to the pod via the `X-Skipper-Function` header when the controller assigns it. Use it for arbitrary data your application needs at runtime (configuration values, version tags, etc.).
