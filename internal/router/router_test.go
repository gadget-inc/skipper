...
func TestGetInstanceDuration(t *testing.T) {
	...
	assert.Assert(t, len(rw.Header().Values(key.GetInstanceDurationMs.Header)) == 1)

	getInstanceDurationMS, err := strconv.ParseInt(rw.Header()[key.GetInstanceDurationMs.Header][0], 10, 64)
	assert.NilError(t, err)
	assert.Assert(t, getInstanceDurationMS >= 20)
}
...
func TestHeartbeats(t *testing.T) {
	...
	mcc.HandleHeartbeat(func(ctx context.Context, routerIP string, heartbeats []*function.Heartbeat, forwardedFor ...string) error {
		...
		assert.Assert(t, heartbeat.GetFunction().Hash() == fn.Hash())
