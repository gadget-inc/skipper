...
// Supervisor manages scaling decisions for a single assignment. It tracks
...
// loop so that assignments don't block each other.
...
// supervisor returns the supervisor for the given assignment, creating it if necessary.
...
// ensureSupervisor returns the supervisor for the given assignment, creating it if
...
// updateAssignment updates the supervisor's assignment if the identity matches
...
// discoverSupervisors discovers assignments in the given namespace by listing
// assigned pods and ensures supervisors exist for each unique assignment.
// Supervisors are created even if this controller is not responsible for the
// assignment, as they track scaling decisions that may be needed if responsibility
// changes. Invalid pods (e.g., with malformed assignment annotations) are deleted.
...
func (s *Supervisor) converge(ctx context.Context) error {
	...
	if fn.GetOneshot() {
		...
		for _, inst := range instances {
			err := s.ctrl.deletePod(ctx, inst.GetAssignment().GetNamespace(), inst.GetName(), metav1.DeleteOptions{})
			if err != nil {
	...
	if isScalingDown {
		...
		// 2. Receive heartbeats from routers (HeartbeatTimeout) - without this, assignments with
	...
	// currentInstances guard, a heartbeat-timeout sweep on an assignment
	// that never had pods bound would emit a misleading "scaling to
	// 0" event.
...
func (s *Supervisor) scaleWithoutLock(ctx context.Context, fn *skipper.Assignment, instances []*skipper.Instance, decision *skipper.ScaleDecision) ([]*skipper.Instance, []*skipper.Instance, error) {
	...
	if desired > ready {
		// we need to scale up
		if total >= maxInstances+1 {
			// we have too many instances in total, so we can't scale up
			log.Info(ctx, "skipping scale up because assignment has too many instances")
			return readyInstances, unreadyInstances, nil
		}
		...
		for _, unreadyInstance := range unreadyInstances {
			err := s.ctrl.deletePod(ctx, unreadyInstance.GetAssignment().GetNamespace(), unreadyInstance.GetName(), metav1.DeleteOptions{})
			if err != nil {
		...
		for toDelete := ready - desired; toDelete > 0; toDelete-- {
			...
			err := s.ctrl.deletePod(ctx, instance.GetAssignment().GetNamespace(), instance.GetName(), metav1.DeleteOptions{})
			if err != nil {
				return nil, nil, fmt.Errorf("failed to delete pod: %w", err)
			}
			s.ctrl.events.add(fn, skipper.EventType_EVENT_TYPE_POD_DELETED, skipper.EventSeverity_EVENT_SEVERITY_INFO,
...
func (s *Supervisor) cleanupStuckInstances(ctx context.Context, fn *skipper.Assignment, instances []*skipper.Instance) []*skipper.Instance {
	...
	return slices.DeleteFunc(instances, func(instance *skipper.Instance) bool {
		if !instance.HasReadyAt() && time.Since(instance.GetAssignedAt().AsTime()) > stuckThreshold {
			...
			s.ctrl.events.add(instance.GetAssignment(), skipper.EventType_EVENT_TYPE_STUCK_INSTANCE_CLEANUP, skipper.EventSeverity_EVENT_SEVERITY_WARN,
...
// getReadyInstance returns a ready instance for the assignment, scaling
...
func (s *Supervisor) getReadyInstance(ctx context.Context, excludeNames []string) (*skipper.Instance, error) {
	...
	for len(instances) == 0 {
		...
		if instances, err = s.scale(ctx, decision); err != nil {
			return nil, fmt.Errorf("failed to scale assignment: %w", err)
		}
	}
	...
	if len(instances) > int(maxInstances) {
		...
		// keep the newest instances up to the max instances allowed for the assignment
...
func calculateDesiredInstancesForMetric(_ context.Context, fn *skipper.Assignment, cfg *Config, metric Metric, instances []*skipper.Instance) (int, float64) {
	...
	for _, instance := range instances {
		...
		if metric == MetricCPU && (!instance.HasReadyAt() || time.Since(instance.GetReadyAt().AsTime()) <= readinessDelay) {
...
func calculateDesiredInstances(ctx context.Context, fn *skipper.Assignment, cfg *Config, heartbeat *skipper.Heartbeat, instances []*skipper.Instance) *skipper.ScaleDecision {
	...
	// Regular assignments use the max across all configured metrics,
	...
	if targetIFR > 0 {
		desiredInstances := int(math.Ceil(float64(heartbeat.GetInFlightRequests()) / float64(targetIFR)))
	...
	clampedValue := min(max(uint32(maxDesiredInstances), minInst), maxInst)
