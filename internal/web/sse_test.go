...
func TestSSEEndpoints(t *testing.T) {
	...
	sseRoutes := []struct {
		...
		{name: "functions", path: "/sse/functions"},
		{name: "function", path: "/sse/function/default%3Aweb-app%3Atenant-1"},
...
func TestSSEFunctionNotFound(t *testing.T) {
	t.Parallel()
	srv := New(func(ctx context.Context) *skipper.ClusterState {
		return testState()
	})

	req := httptest.NewRequest(http.MethodGet, "/sse/function/nonexistent", nil)
	w := httptest.NewRecorder()
	srv.Handler().ServeHTTP(w, req)
...
func TestSSEFunctionsFragments(t *testing.T) {
	t.Parallel()
	srv := New(func(ctx context.Context) *skipper.ClusterState {
		return testState()
	})

	req := httptest.NewRequest(http.MethodGet, "/sse/functions", nil)
	w := httptest.NewRecorder()
	srv.Handler().ServeHTTP(w, req)
	...
	assert.Assert(t, strings.Contains(body, "datastar-patch-elements"),
		"should contain datastar event type")
	assert.Assert(t, strings.Contains(body, "functions-table"),
		"should contain functions-table selector")
}
