...
func TestNew_BasicShapes(t *testing.T) {
	...
	t.Run("Attr.Otel flattens group with dot notation", func(t *testing.T) {
		attr := k.Attr(v)
		assert.Equal(t, len(attr.Otel), 1)
		assert.Equal(t, attr.Otel[0].Key, attribute.Key("greeting.name"))
		assert.Equal(t, attr.Otel[0].Value.AsString(), "hello")
	})
	...
	t.Run("Names is embedded", func(t *testing.T) {
...
// testOtelSource implements LogValue + OtelAttrs so NewWithOtel can wire
// it. Used only by TestNewWithOtel below.
...
func TestNewWithOtel(t *testing.T) {
	t.Parallel()
	k := NewWithOtel("override", (*testOtelSource).LogValue, (*testOtelSource).OtelAttrs)
	...
	t.Run("Attr.Slog uses LogValue, Attr.Otel uses otelOf override", func(t *testing.T) {
		attr := k.Attr(v)
		assert.Equal(t, attr.Slog.Value.String(), "hi")
		assert.Equal(t, len(attr.Otel), 1)
		assert.Equal(t, attr.Otel[0].Key, attribute.Key("custom.key"))
		assert.Equal(t, attr.Otel[0].Value.AsString(), "hi-otel")
	})
}
