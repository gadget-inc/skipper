...
var messageTableDescriptions = map[string]string{
	...
	"Assignment.scale":      "Per-assignment scaling targets (min, max, CPU, memory, in-flight requests). Flat-form `scale_*` fields take precedence when both are set.",
	"Assignment.oneshot":    "True if each request gets a fresh pod; assigned pods are released after the request completes.",

	"Assignment.scale_min_instances":             "Minimum ready-instance floor (flat alias for `scale.min_instances`).",
	"Assignment.scale_max_instances":             "Hard ceiling on ready instances (flat alias for `scale.max_instances`).",
	"Assignment.scale_target_cpu_millicores":     "Per-instance CPU target in millicores (flat alias for `scale.target_cpu_usage_milli`).",
	"Assignment.scale_target_memory_mebibytes":   "Per-instance memory target in mebibytes (flat alias for `scale.target_memory_usage_mib`).",
	"Assignment.scale_target_in_flight_requests": "Per-instance in-flight-request target (flat alias for `scale.target_in_flight_requests`).",
	"Assignment.scale_tolerance":                 "HPA usage-ratio tolerance; usage within this band of the target produces no scale change.",
	"Assignment.scale_downscale_stabilization":   "Downscale stabilization window; the supervisor records recommendations over this period and scales down to the maximum within it.",
	"Assignment.scale_initial_readiness_delay":   "Per-pod initial-readiness delay during which CPU metrics are ignored.",

	"Assignment.zone_spread":   "**Planned (followup: zone-aware-placement).** Spread-across-zones policy. Setting this field has no effect in the current release.",
	"Assignment.zone_min":      "**Planned (followup: zone-aware-placement).** Minimum number of zones to spread across. Setting this field has no effect in the current release.",
	"Assignment.zone_affinity": "**Planned (followup: zone-aware-affinity).** Same-zone routing preference. Setting this field has no effect in the current release.",

	"Assignment.assign_path":      "**Planned (followup: per-assignment-assign-path).** Override the controller's `/__skipper/assign` POST path. Setting this field has no effect in the current release.",
	"Assignment.assign_timeout":   "Per-assignment override for the controller's assign POST timeout.",
	"Assignment.assign_token_ttl": "Per-assignment override for the PASETO token's time-to-live.",

	"Assignment.heartbeat_interval": "**Planned (followup: per-assignment-heartbeat-interval).** Override the router's heartbeat send interval. Setting this field has no effect in the current release.",
	"Assignment.heartbeat_timeout":  "Per-assignment override for the controller's heartbeat timeout (scale-to-zero and pod-cleanup decisions).",

	"Assignment.retry_max_attempts": "Per-assignment override for the router's max round-trip retry attempts.",
	"Assignment.retry_min_backoff":  "Per-assignment override for the router's minimum retry backoff.",
	"Assignment.retry_max_backoff":  "Per-assignment override for the router's maximum retry backoff.",
	"Assignment.retry_backpressure": "**Planned (followup: backpressure).** Backpressure response policy. Setting this field has no effect in the current release.",
	"Assignment.retry_status_codes": "**Planned (followup: backpressure).** HTTP status codes that should trigger a retry. Setting this field has no effect in the current release.",

	"Assignment.transport_dial_timeout":          "**Planned (followup: per-assignment-transport).** Transport dial timeout. Setting this field has no effect in the current release.",
	"Assignment.transport_keepalive":             "**Planned (followup: per-assignment-transport).** Transport TCP keepalive interval. Setting this field has no effect in the current release.",
	"Assignment.transport_idle_conn_timeout":     "**Planned (followup: per-assignment-transport).** Transport idle-connection timeout. Setting this field has no effect in the current release.",
	"Assignment.transport_tls_handshake_timeout": "**Planned (followup: per-assignment-transport).** Transport TLS handshake timeout. Setting this field has no effect in the current release.",
	"Assignment.transport_max_idle_conns":        "**Planned (followup: per-assignment-transport).** Transport max idle connections. Setting this field has no effect in the current release.",
	"Assignment.transport_force_http2":           "**Planned (followup: per-assignment-transport).** Force HTTP/2 on the transport. Setting this field has no effect in the current release.",
	"Assignment.transport_disable_compression":   "**Planned (followup: per-assignment-transport).** Disable transport-level compression. Setting this field has no effect in the current release.",
	"Assignment.transport_flush_interval":        "**Planned (followup: per-assignment-transport).** Reverse-proxy response flush interval. Setting this field has no effect in the current release.",
