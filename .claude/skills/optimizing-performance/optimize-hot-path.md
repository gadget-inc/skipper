...
dev tests go -bench=BenchmarkName -benchmem -count=6 ./internal/<package>/... | tee tmp/benchmarks/<name>-before.txt
```
...
```bash
dev tests go -short ./internal/<package>/...
```
...
```bash
dev tests go -bench=BenchmarkName -benchmem -count=6 ./internal/<package>/... | tee tmp/benchmarks/<name>-after.txt
```
