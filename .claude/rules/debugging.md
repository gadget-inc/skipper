...
When `dev` is running, structured JSON logs are written to:
...
Router integration tests send requests to the local router at `http://127.0.0.1:8081` (override with `SKIPPER_ROUTER_URL`). These require `dev` to be running with at least the controller and router, and fixtures deployed (`deploy --only=fixtures`).
